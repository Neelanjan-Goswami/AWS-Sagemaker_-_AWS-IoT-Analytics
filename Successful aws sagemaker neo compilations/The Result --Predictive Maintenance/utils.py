...
#         train_df.append(df)
    ...
    df = pd.read_csv('{}/train_FD{:03d}.txt'.format(data_location ,1), delimiter=' ', header=None)
    df.drop(df.columns[[26, 27]], axis=1, inplace=True)
    df.columns = columns
    df[columns[2:]] = (df[columns[2:]] - df[columns[2:]].min() + eps) / (
    df[columns[2:]].max() - df[columns[2:]].min() + eps)
    ...
    #for i in range(1, 5):
        # Load time series
    test_df = pd.read_csv('{}/test_FD{:03d}.txt'.format(data_location, 1), delimiter=' ', header=None)
    test_df.drop(test_df.columns[[26, 27]], axis=1, inplace=True)

        # Load the RUL values
    df_rul = pd.read_csv('{}/RUL_FD{:03d}.txt'.format(data_location, 1), delimiter=' ', header=None)
    df_rul.drop(df_rul.columns[1], axis=1, inplace=True)
    df_rul.index += 1

        # Merge RUL and timeseries and compute RUL per timestamp
    test_df = test_df.merge(df_rul, left_on=df.columns[0], right_index=True, how='left')
