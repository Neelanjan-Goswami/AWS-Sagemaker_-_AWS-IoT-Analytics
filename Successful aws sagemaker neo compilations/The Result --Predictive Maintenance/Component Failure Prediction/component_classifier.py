...
def model_fn(features, labels, mode, params):
    ...
    input_layer = tf.reshape(features[INPUT_TENSOR_NAME], [-1,31])
    ...
    if mode in (Modes.TRAIN, Modes.EVAL):
        ...
        loss = tf.losses.softmax_cross_entropy(
            onehot_labels=tf.one_hot(label_indices, depth=5), logits=logits)
...
def serving_input_fn(params):
    inputs = {INPUT_TENSOR_NAME: tf.placeholder(tf.float32, [1,31], name = 'data')}
