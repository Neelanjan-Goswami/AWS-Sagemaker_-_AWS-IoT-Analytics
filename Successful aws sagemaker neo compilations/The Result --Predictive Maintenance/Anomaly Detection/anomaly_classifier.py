...
def model_fn(features, labels, mode, params):
    ...
    dense1 = tf.layers.dense(inputs=input_layer, units=32, activation=tf.nn.relu)
    ...
    logits = tf.layers.dense(inputs=dense3, units=2, activation=None)
...
#             probabilities=1
    ...
    if mode in (Modes.TRAIN, Modes.EVAL):
        ...
        loss = tf.losses.softmax_cross_entropy(
            onehot_labels=tf.one_hot(label_indices, depth=2), logits=logits)
...
def read_and_decode(filename_queue):
    ...
    image = tf.decode_raw(features['feature_raw'], tf.float64)
    image.set_shape([4])
